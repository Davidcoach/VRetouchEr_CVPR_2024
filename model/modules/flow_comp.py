...
class FlowCompletionLoss(nn.Module):
    ...
    def forward(self, pred_flows, gt_local_frames):
        ...
        with torch.no_grad():
            ...
            gt_local_frames = F.interpolate(gt_local_frames.view(-1, c, h, w), scale_factor=1 / 4, mode='bilinear',align_corners=True,recompute_scale_factor=True)
        ...
        forward_flow_loss = self.l1_criterion(pred_flows[0].view(-1, 2, h // 4, w // 4), gt_flows_forward)
        backward_flow_loss = self.l1_criterion(pred_flows[1].view(-1, 2, h // 4, w // 4), gt_flows_backward)
        flow_loss = forward_flow_loss + backward_flow_loss
        return flow_loss


class BlemishFlow_Loss(nn.Module):
    """Flow completion loss"""
    def __init__(self, device):
        super().__init__()
        self.device = device
        self.fix_spynet = SPyNet().to(self.device)
        for p in self.fix_spynet.parameters():
            p.requires_grad = False
        self.l1_criterion = nn.L1Loss()
    ...
    def forward(self, pred_flows, input_img_list, gt_img_list):
        diff_list = []
        loss = 0
        for i in range(len(input_img_list)):
            input_img = F.interpolate(input_img_list[i], scale_factor=1 / 8, mode='bilinear',align_corners=True, recompute_scale_factor=True)
            gt_img = F.interpolate(gt_img_list[i], scale_factor=1 / 8, mode='bilinear',align_corners=True, recompute_scale_factor=True)
            diff = torch.abs(gt_img - input_img).mean(dim=1, keepdim=True)
            ...
            diff_list.append(diff_normed)
        # m = torch.cat(diff_list, dim=0)
        # save_image(m, f"source.png", normalize=True, value_range=(-1, 1))
        diff = torch.stack(diff_list, dim=1).to(self.device)
        b, l_t, c, h, w = diff.size()

        with torch.no_grad():
            # compute gt forward and backward flows
            gtlf_1 = diff[:, :-1, :, :, :].reshape(-1, c, h, w)
            gtlf_2 = diff[:, 1:, :, :, :].reshape(-1, c, h, w)
            gt_flows_forward = self.fix_spynet(gtlf_1, gtlf_2)

        # for flow in gt_flows_forward:
        #     img = flow_to_image(flow)
        #     f_list.append(img)
        # flow = np.concatenate(f_list, axis=1)
        # import cv2
        # cv2.imwrite("visual.png", flow)
        #
        # f_list = []
        # for flow in pred_flows:
        #     img = flow_to_image(flow[0].detach().reshape(-1, 64, 64))
        #     f_list.append(img)
        # flow = np.concatenate(f_list, axis=1)
        # import cv2
        # cv2.imwrite("visual_.png", flow)
        for i in range(5):
            gt = gt_flows_forward[i]
            flow = pred_flows[i]
            loss += self.l1_criterion(flow.reshape(2, h, w), gt.reshape(2, h, w))

        # forward_flow_loss = self.l1_criterion(pred_flows.view(-1, 2, h, w), gt_flows_forward.view(-1, 2, h, w))
        return loss
...
class SPyNet(nn.Module):
    ...
    def __init__(
        self,
        use_pretrain=True,
        pretrained="./model/modules/spynet_20210409-c6c1bd09.pth"
    ):
        super().__init__()
...
def flow_to_image(flow_uv, clip_flow=None, convert_to_bgr=False):
    ...
    if clip_flow is not None:
        flow_uv = np.clip(flow, 0, clip_flow)
    u = flow[0, :, :]
    v = flow[1, :, :]
    rad = np.sqrt(np.square(u) + np.square(v))
    rad_max = np.max(rad)
...
def flow_warp(x,
    ...
    n, c, h, w = x.size()
    flow = torch.reshape(flow, shape=(n, h, w, 2))
    # create mesh grid
